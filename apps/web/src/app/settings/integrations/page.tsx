...
export default function IntegrationSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* HR & Workforce */}
      <div>
        <h2 className="text-lg font-bold mt-8 mb-1">HR & Workforce</h2>
        <p className="text-sm text-slate-500 mb-4">Employee management, utilization tracking, and PTO</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader><div className="flex items-center gap-3"><Scale className="h-6 w-6 text-indigo-600" /><div><CardTitle className="text-sm">Rippling</CardTitle><CardDescription className="text-xs">Sync employee data, track attorney utilization, manage PTO with conflict detection, monitor CLE compliance, and analyze workforce profitability.</CardDescription></div></div></CardHeader>
          <CardContent><p className="text-xs text-gray-400 mb-3">Rippling syncs employee profiles and time off. Utilization is calculated from Clio AI time tracking data.</p><Button size="sm" variant="outline">Connect Rippling</Button></CardContent>
        </Card>
      </div>
...
function LCSSection({ webhookBase }: { webhookBase: string }) {
  const { toast } = useToast();
  const utils = trpc.useUtils();
  const [configOpen, setConfigOpen] = useState(false);
  const [lForm, setLForm] = useState<any>({ apiKey: "", accountId: "", firmId: "", isEnabled: false, defaultJurisdiction: "NY", defaultTitleCompany: "", autoSyncMatters: true, autoGenerateDocuments: true, autoCalculateAdjustments: true });

  const { data: config } = trpc.conveyancing["settings.get"].useQuery();
  const updateMut = trpc.conveyancing["settings.update"].useMutation({ onSuccess: () => { utils.conveyancing["settings.get"].invalidate(); setConfigOpen(false); toast({ title: "Saved" }); } });
  const testMut = trpc.conveyancing["settings.test"].useMutation({ onSuccess: (d: any) => toast({ title: d.success ? "Connected!" : `Failed: ${d.error}`, variant: d.success ? "default" : "destructive" }) });

  const connected = config?.isEnabled && config?.apiKey;
  const loadForm = () => {
    if (config) setLForm({ apiKey: config.apiKey || "", accountId: config.accountId || "", firmId: config.firmId || "", isEnabled: config.isEnabled, defaultJurisdiction: config.defaultJurisdiction || "NY", defaultTitleCompany: config.defaultTitleCompany || "", autoSyncMatters: config.autoSyncMatters, autoGenerateDocuments: config.autoGenerateDocuments, autoCalculateAdjustments: config.autoCalculateAdjustments });
    setConfigOpen(true);
  };
  ...
  return (
    <>
      <div><h2 className="text-lg font-bold mt-8 mb-1">Conveyancing</h2><p className="text-sm text-slate-500 mb-4">Real estate closing management, document generation, and title clearance</p></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3"><Home className="h-6 w-6 text-blue-600" /><div><CardTitle className="text-sm">Lawyer Conveyance System (LCS)</CardTitle><CardDescription className="text-xs leading-relaxed">Real estate conveyancing document generation, financial management, closing statement preparation, and title clearance tracking.</CardDescription></div></div>
              {connected ? <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0" /> : <XCircle className="h-5 w-5 text-gray-300 flex-shrink-0" />}
            </div>
          </CardHeader>
          <CardContent>
            <p className="text-xs text-gray-400 mb-3">LCS generates deeds, closing statements, transfer tax returns, and conveyancing documents auto-populated from your matter data.</p>
            <div className="flex gap-2"><Button size="sm" variant="outline" onClick={loadForm}>Configure</Button>{connected && <Button size="sm" variant="outline" onClick={() => testMut.mutate()} disabled={testMut.isLoading}>Test</Button>}</div>
          </CardContent>
        </Card>
      </div>
      <Card className="mt-4">
        <CardHeader><CardTitle className="text-sm">Conveyancing Webhook URL</CardTitle></CardHeader>
        <CardContent><div className="flex items-center gap-2"><span className="text-sm text-slate-500 w-20">LCS:</span><code className="flex-1 text-xs bg-slate-100 px-2 py-1 rounded font-mono truncate">{webhookBase}/api/integrations/lcs/webhook</code><Button variant="ghost" size="sm" onClick={() => { navigator.clipboard?.writeText(`${webhookBase}/api/integrations/lcs/webhook`); toast({ title: "Copied" }); }}><Copy className="h-3 w-3" /></Button></div></CardContent>
      </Card>
      {configOpen && (
        <Dialog open onOpenChange={() => setConfigOpen(false)}>
          <DialogContent className="max-h-[80vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Configure LCS</DialogTitle></DialogHeader>
            <div className="space-y-4">
              <Toggle label="Enabled" checked={lForm.isEnabled} onChange={(v: boolean) => setLForm({ ...lForm, isEnabled: v })} />
              <div className="space-y-2"><Label>API Key</Label><Input type="password" value={lForm.apiKey} onChange={(e: any) => setLForm({ ...lForm, apiKey: e.target.value })} /></div>
              <div className="space-y-2"><Label>Account ID</Label><Input value={lForm.accountId} onChange={(e: any) => setLForm({ ...lForm, accountId: e.target.value })} /></div>
              <div className="space-y-2"><Label>Firm ID</Label><Input value={lForm.firmId} onChange={(e: any) => setLForm({ ...lForm, firmId: e.target.value })} /></div>
              <div className="space-y-2"><Label>Default Jurisdiction</Label><Input value={lForm.defaultJurisdiction} onChange={(e: any) => setLForm({ ...lForm, defaultJurisdiction: e.target.value })} placeholder="NY" /></div>
              <div className="space-y-2"><Label>Default Title Company</Label><Input value={lForm.defaultTitleCompany} onChange={(e: any) => setLForm({ ...lForm, defaultTitleCompany: e.target.value })} /></div>
              <Toggle label="Auto-sync matters" checked={lForm.autoSyncMatters} onChange={(v: boolean) => setLForm({ ...lForm, autoSyncMatters: v })} />
              <Toggle label="Auto-generate documents" checked={lForm.autoGenerateDocuments} onChange={(v: boolean) => setLForm({ ...lForm, autoGenerateDocuments: v })} />
              <Toggle label="Auto-calculate adjustments" checked={lForm.autoCalculateAdjustments} onChange={(v: boolean) => setLForm({ ...lForm, autoCalculateAdjustments: v })} />
              <Button className="w-full" disabled={updateMut.isLoading} onClick={() => updateMut.mutate({ apiKey: lForm.apiKey || null, accountId: lForm.accountId || null, firmId: lForm.firmId || null, isEnabled: lForm.isEnabled, defaultJurisdiction: lForm.defaultJurisdiction || null, defaultTitleCompany: lForm.defaultTitleCompany || null, autoSyncMatters: lForm.autoSyncMatters, autoGenerateDocuments: lForm.autoGenerateDocuments, autoCalculateAdjustments: lForm.autoCalculateAdjustments })}>
                {updateMut.isLoading ? "Saving..." : "Save Configuration"}
              </Button>
            </div>
          </DialogContent>
